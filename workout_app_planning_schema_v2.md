...
```txt
Full-stack app: Next.js App Router + TypeScript
...
This replaces the earlier recommended React + Vite + FastAPI stack.

The rest of this document remains the source of truth for product behavior and data modeling:
...
RPE behavior
finish validation
autosync expectations
REST/API shape
implementation warnings
```

Only the runtime/application chassis changed.

---

## 2.2 Why Next.js Fits This App

This is a single-user, solo-developer, self-hosted app.

A split frontend/backend stack such as React + Vite + FastAPI is still valid, but it creates duplicate modeling work:
...
With Next.js + Prisma, the project can keep the app in one TypeScript codebase:

```txt
Prisma schema -> generated Prisma client -> API route/server action -> React components
```

This reduces duplication and makes iteration faster for a solo developer.

---

## 2.3 Role of Next.js

Next.js is not being chosen because the app requires server-side rendering.

The app is still primarily an interactive client-side mobile web app.

Next.js is being used because it provides:

- React UI
- file-based routing
- built-in backend/API route capability
- one TypeScript project
- simple deployment as one app container

The app can still behave like a normal client-heavy workout tracker.

Most screens, especially the live workout screen, should be implemented as client components where needed.
...
Prisma should represent the database schema described in this specification, including:
...
- weight unit fields
- cascade behavior
- `ON DELETE SET NULL` behavior where needed
- partial unique index for one active workout

Important note:

Some PostgreSQL-specific constraints and indexes may require manual SQL migrations in Prisma.
...
partial unique index for one active workout
pgcrypto extension
advanced CHECK constraints if Prisma cannot express them directly
```

These database rules still matter even if Prisma is used.
...
This is unchanged from the earlier recommendation.

The app should still use relational tables for:
...
Do not store completed workouts only as JSON blobs.

Relational data is required for future features such as:
...
Tailwind remains the styling approach.

The app is mobile-first and needs:
...
- compact editable set rows
- responsive workout screens
...
This is simpler than the earlier three-container setup:
...
The app container runs the Next.js application.

The postgres container stores persistent data.

---

## 2.8 What Changed and What Did Not

Changed:

```txt
React + Vite frontend -> Next.js App Router
FastAPI backend -> Next.js API routes / server-side handlers
SQLAlchemy/Alembic -> Prisma migrations/client
three containers -> two containers
```
...
all workout behavior
all set type rules
all finish/save/edit behavior
single-user scope
```

---

## 2.9 Alternative Stack Still Valid

The earlier React + Vite + FastAPI stack remains a valid alternative, especially for teams that prefer a clean frontend/backend separation.

Pros of React + Vite + FastAPI:

- very explicit backend/frontend separation
- FastAPI has excellent request validation
- Python is strong for backend service logic
- good if backend may grow into a separate API service
...
- more containers
- more boilerplate for a solo developer

For this specific single-user app, Next.js + Prisma is an acceptable and practical choice.
...
# 16. Prisma Implementation Documentation

This section translates the finalized PostgreSQL schema into a practical Prisma implementation for the chosen stack:

```txt
Next.js App Router + TypeScript + Prisma + PostgreSQL
```

The goal is to let Prisma provide generated TypeScript types and a clean query API while preserving the PostgreSQL rules that Prisma cannot fully express by itself.

Important principle:

```txt
Prisma models describe the application data shape.
PostgreSQL constraints enforce the product rules.
```

If Prisma cannot express a constraint, use raw SQL inside the Prisma migration.

---

## 16.1 Prisma Generator and Datasource

Prisma 7 uses the new `prisma-client` generator.

The project convention is:

```txt
Prisma client output: src/generated/prisma
Import path: @/generated/prisma
Config file: prisma.config.ts
```

Recommended `schema.prisma` header:
...
## 16.2 Prisma Enum Definitions

Use Prisma enums where appropriate. Still add PostgreSQL CHECK constraints in raw SQL if stronger database enforcement is needed.
...
- `WorkoutStatus` does not include discarded because discarded active workouts are hard-deleted.
- Saved workouts are hard-deleted when deleted.
- `WeightUnit` is used for the app-wide default and per-set entered unit.

---

## 16.3 Prisma Schema

The following schema is the recommended Prisma translation of the finalized data model.
...
model WorkoutSessionExercise {
  ...
  sets           WorkoutSet[]    @relation("WorkoutExerciseSets")
...
model WorkoutSet {
  ...
  weightInDefaultUnit      Decimal?   @map("weight_in_default_unit") @db.Decimal(10, 2)
  reps                     Int?
  ...
  workoutSessionExercise WorkoutSessionExercise @relation("WorkoutExerciseSets", fields: [workoutSessionExerciseId], references: [id], onDelete: Cascade)
  parentSet              WorkoutSet?            @relation("DropSetParent", fields: [parentSetId], references: [id], onDelete: SetNull)
  childDropSets          WorkoutSet[]           @relation("DropSetParent")
...
## 16.4 Important Prisma Modeling Notes

### Single-user schema

The MVP is single-user and has no auth.

Therefore, the schema above omits `user_id`.

If authentication is added later, add ownership columns intentionally and update all route handlers accordingly.

Do not add multi-user/social assumptions now.

### Exercise deletion

`WorkoutSessionExercise.exerciseId` is nullable and uses `ON DELETE SET NULL`.

This allows hard deleting an exercise while preserving saved workout history through snapshots.

### Snapshot fields

When adding an exercise to a workout, always snapshot:
...
The saved workout view should primarily use these snapshot fields for display.

### Set number vs row index

`rowIndex` and `setNumber` are separate.

Do not collapse them.

Warmup and drop-set behavior depends on this separation.
...
```txt
set_type = drop
parent_set_id = previous numbered/failure set id
set_number = null
```

This models the finalized rule:

```txt
Drop set attaches to previous numbered/failure set and does not consume a new set number.
```

---

## 16.5 Required Raw SQL Migration Parts

Prisma should generate most tables and relations, but the migration must include raw SQL for PostgreSQL-specific behavior.
...
Place this at the top of the initial migration before tables using `gen_random_uuid()`.

---

### Enforce one active workout

Only one active workout may exist in this single-user app.

Prisma cannot express partial unique indexes directly in the model.
...
### App settings single-row constraint

`AppSettings.id` defaults to 1, but PostgreSQL should enforce that it can only be 1.
...
### RPE half-step validation

The Prisma schema can represent range limits with manual CHECK constraints.

Use this if you want database-level enforcement of 0.5 increments:
...
ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_rpe_half_step
CHECK (rpe IS NULL OR (rpe * 2 = floor(rpe * 2)));
```

This enforces allowed values such as:
...
Application validation should still enforce this before writing.

---

### Weight and reps non-negative constraints

The app allows:

```txt
weight = 0
reps = null while editing
reps = 0 while editing but invalid for recording
```

Use database constraints to block impossible negative values:

```sql
ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_weight_non_negative
CHECK (weight IS NULL OR weight >= 0);

ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_weight_default_non_negative
CHECK (weight_in_default_unit IS NULL OR weight_in_default_unit >= 0);
...
Do not enforce `reps >= 1` at database level because rows may be incomplete during live editing.

Finish validation enforces the recorded-set rules.

---

### Set numbering consistency constraints

Some set type rules are easier to enforce in service logic than in SQL.

Still, add lightweight constraints:

```sql
ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_warmup_has_no_set_number
CHECK (set_type <> 'warmup' OR set_number IS NULL);

ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_drop_has_no_set_number
CHECK (set_type <> 'drop' OR set_number IS NULL);

ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_normal_has_set_number
CHECK (set_type <> 'normal' OR set_number IS NOT NULL);

ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_failure_has_set_number
CHECK (set_type <> 'failure' OR set_number IS NOT NULL);
```

The exact sequencing and renumbering still belongs in application/service logic.

### Drop parent rule

A drop set should attach to a previous numbered/failure set.

This is hard to fully enforce with a simple CHECK constraint because it depends on another row's type and position.

Enforce in service logic:

```txt
if set_type = drop:
  parent_set_id must point to a previous set in the same workout exercise
  parent set must be normal or failure
```

Optionally add this weaker database constraint:

```sql
ALTER TABLE workout_sets
ADD CONSTRAINT workout_sets_drop_requires_parent
CHECK (set_type <> 'drop' OR parent_set_id IS NOT NULL);
```

If you want to allow temporary invalid rows while editing set type, do not add this constraint until the service flow is stable.

---

## 16.6 Seed Data Migration

Seed reference data in the initial migration or a seed script.
...
## 16.7 Recommended Initial Migration Workflow

Use Prisma normally, then edit the generated SQL migration before applying.

Recommended flow:

```sh
npx prisma migrate dev --create-only --name init_core_schema
```

Then edit the generated migration SQL and add:
...
## 16.8 Service Logic Required Beyond Prisma

Do not rely on Prisma schema alone for behavior.

Application/service logic must handle:
...
Also rely on the database partial unique index for race-condition safety.
...
```txt
assign next row_index
default set_type = normal
recalculate set_number values
```
...
```txt
update set_type
if drop, attach parent_set_id to previous numbered/failure set
recalculate set_number values for all rows in that workout exercise
```
...
delete row
reindex row_index
recalculate set_number
repair/drop invalid parent_set_id references
```
...
```txt
at least one set has reps >= 1
invalid weighted rows detected before save screen
no invalid weighted rows remain on final save
```

### Edit completed workout validation

Must enforce the same validation rules as live workout.

Do not allow saving an edited completed workout if all sets become invalid.

---

## 16.9 Prisma Client Usage Expectations

Use Prisma generated types end-to-end where practical.

Use a singleton Prisma client in Next.js development to avoid connection exhaustion.
...
Adjust the import path to match the generated Prisma 7 output used in this project.

---

## 16.10 What Not To Do With Prisma

Do not:

```txt
store completed workouts as JSON blobs
remove PostgreSQL constraints because Prisma cannot express them
filter saved sets by checked = true
collapse row_index and set_number
skip snapshot fields
allow multiple active workouts
rely only on client-side validation
```

The Prisma implementation must preserve the product rules.
